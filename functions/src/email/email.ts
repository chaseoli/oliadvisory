...
import { HTMLEmailTemplates } from '../email/templates/html-templates';
import { PlainEmailTemplates } from '../email/templates/plain-templates';
...
export class Email {
    ...
    constructor(
        ...
        apiKey.apiKey = new Constants().sendInBlueApiKey;
    ...
    htmlEmail(htmlBody: string, ): string {
        return this.htmlTemplates.defaultEmail().replace('[BODY]', htmlBody);
    }
    ...
    textEmail(body: string): string {
        return this.plainTemplates.defaultEmail().replace('[BODY]', body);
    }
